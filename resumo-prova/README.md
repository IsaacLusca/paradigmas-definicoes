# resumo-prova

Material de estudo para as provas de Paradigmas de Programação.

## Estrutura

```
resumo-prova/
├── estudo/
│   └── RESUMO_PROVA_1.md      ← RESUMO GERAL ÚNICO para a Prova 1 (comece por aqui)
├── detalhado/                 ← explicação profunda (debug linha a linha + passo a passo)
│   ├── SIMULADO_1_DETALHADO.md ... SIMULADO_4_DETALHADO.md
│   └── LISTA_PL_DETALHADA.md / LISTA_COMBINADORES_DETALHADA.md
├── simulador.html             ← simulador dos 4 simulados + listas com resposta comentada (abra no navegador)
├── simulados/
│   ├── SIMULADO_1.md          ← simulado no estilo da prova (12 MC + 2 abertas)
│   ├── SIMULADO_1_GABARITO.md ← gabarito comentado
│   ├── SIMULADO_2.md
│   ├── SIMULADO_2_GABARITO.md
│   ├── SIMULADO_3.md
│   ├── SIMULADO_3_GABARITO.md
│   ├── SIMULADO_4.md          ← Prova 1 real (4BAE41) + Combinadores (704C60): 10 MC + 2 CDU + Parte C
│   └── SIMULADO_4_GABARITO.md
├── prova_1/
│   ├── prova_1_4BAE41.pdf     ← Prova 1 aplicada (Prolog + lógica)
│   ├── gabarito_4BAE41.csv    ← respostas oficiais
│   └── GABARITO.md            ← respostas legíveis + justificativas
├── prova_2/
│   ├── prova_2_704C60.pdf     ← Prova 2 antiga (Combinadores + Lambda + Haskell)
│   ├── gabarito_704C60.csv    ← respostas oficiais
│   └── GABARITO.md            ← respostas legíveis + justificativas
└── listas/
    ├── lista_programacao_logica.pdf   ← exercícios de Prolog (partes A-D)
    └── lista_combinadores.pdf         ← exercícios de Combinadores/Base SK
```

## Para a Prova 1, cai

- **Programação Lógica (Prolog):** aulas 01 a 06 (lógica booleana, primeira ordem, backtracking,
  aritmética, recursão, unificação).
- **Combinadores:** Introdução e Base SK (partes da antiga Prova 2).
- **Não cai:** Cálculo Lambda e Haskell.

> **Formato (avisado em aula): sem escrita de código** — no máximo código para analisar.
> Simulados 1–3: Parte A (12 MC, 2 pts) + Parte B (2 abertas em CDU, 3 pts) = 30 pts.
> Simulado 4: fiel à prova real — Parte A (10 MC, 2 pts) + Parte B (2 CDU, 2 pts) +
> Parte C de Combinadores/análise (6 × 1 pt) = 30 pts.
> As **listas** do curso estão no `simulador.html` com resposta comentada (sem pontuação).
> Veja a seção 0 do `estudo/RESUMO_PROVA_1.md`.
